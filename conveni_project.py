...
from openpyxl.styles import Alignment #AEAEAE #20200329
...
#maplist = openpyxl.load_workbook('c:/Users/csw44/Desktop/입점사 정보.xlsx')
maplist = openpyxl.load_workbook('c:/Users/csw44/Desktop/입점사 정보 - 테스트용.xlsx')
...
# for i in range(len(ceo_email_list)):
#     print(str(ceo_name_list[i]) + ' : ' + str(ceo_email_list[i]))

# 열간격 수정 20200329
column_list_width = [16,6,19,19,8,12,14,95,9,6,8,14,14,80,28,9]
...
for i in range(len(supplier)):
    ...
    sheet = wb.active
    sheet.append(title) #20200329 가운데 정렬, 회색 색깔 처리
    #sheet['A1'].fill(PatternFill(start_color='AEAEAE', end_color='AEAEAE'))
    ...
    for j in range(len(origin)):
        if origin[j][4] == supplier[i]:
            ...
            continue
company_final_list = []
email_final_list = [] # 이메일주소 저장을 위해 생성
print(len(supplier))
print(len(ceo_name_list))
print(len(ceo_email_list))
#파일명 변경 프로세스
for i in range(len(supplier)): #공급자명 리스트 for 문
    for j in range(len(ceo_name_list)): #ceo_name_list for 문
        if supplier[i] == ceo_name_list[j]: #공급자명과 ceo_name_list 비교
            ...
            email_final_list.append(ceo_email_list[j]) # 20200401 추가
            for m in range(len(email_final_list)):
                print('공급자명 : ' + supplier[i] + ', ' + '대표명 : ' + ceo_name_list[j] + ', ' + '회사명 : ' + company_name_list[j]
                        + ' ' + '이메일 주소 : ' + email_final_list[m])
                break
            try :
              rename('C:/Users/csw44/Desktop/컨비니_{}_{}_{}_{}_발주서.xlsx'.format(year, month, day, ceo_name_list[j]),
                       '컨비니_{}_{}_{}_{}_발주서.xlsx'.format(year, month, day, company_name_list[j]))
            except:
...
#print(email_final_list)

for i in range(len(email_final_list)):
    print(str(email_final_list[i]) + '으로 메일을 발송합니다.')
    sending_email(email_final_list[i], filenamelist[i])
    print('메일 발송 완료')
